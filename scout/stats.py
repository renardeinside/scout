...
def ttest_main(args):
    ...
    # Compute p-values
    t, p = stats.ttest_ind(
        a, b, axis=1, equal_var=True, nan_policy="omit"
    )  # Can 'propagate' nans too
    neg_logp = -np.log10(p)
...
def ttest_cli(subparsers):
    ttest_parser = subparsers.add_parser(
        "t-test",
        help="Perform independent t-tests",
        description="Performs independent t-tests on two groups of organoid features",
    )
    ttest_parser.add_argument(
        "-a", help="Path to combined feature table for group A", required=True
    )
    ttest_parser.add_argument(
        "-b", help="Path to combined feature table for group B", required=True
    )
    ttest_parser.add_argument(
        "-o", "--output", help="Path to excel spreadsheet with results", required=True
    )
...
def stats_cli(subparsers):
    stats_parser = subparsers.add_parser(
        "stats",
        help="statistical analyses",
        description="Perform statstical analyses on organoid features",
    )
    stats_parser = stats_parser.add_subparsers(
        dest="stats_command", title="stats subcommands"
    )
