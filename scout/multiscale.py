...
def singlecell_features(
    args, features, gate_labels, niche_labels, nuclei_morphologies, niche_proximities
):
    ...
    for n, niche_name in enumerate(niches):
        ...
        for c, celltype_name in enumerate(celltypes):
            ...
            print(
                "SOX2, TBR1",
                proximities_celltype.mean(axis=0),
                proximities_celltype.std(axis=0),
                f"n = {len(celltype_idx)}",
            )
            mean_proximities = proximities_celltype.mean(axis=0)
            stdev_proximities = proximities_celltype.std(axis=0)
            features[
                f"{niche_name} nbrhd, {celltype_name} proximity to SOX2 mean"
            ] = mean_proximities[0]
            features[
                f"{niche_name} nbrhd, {celltype_name} proximity to SOX2 stdev"
            ] = stdev_proximities[0]
            features[
                f"{niche_name} nbrhd, {celltype_name} proximity to TBR1 mean"
            ] = mean_proximities[1]
            features[
                f"{niche_name} nbrhd, {celltype_name} proximity to TBR1 stdev"
            ] = stdev_proximities[1]
...
def cytoarchitecture_features(args, features):
    ...
    for c, cyto_name in enumerate(cytoarchitectures):
        ...
        if len(cyto_idx) == 0:
            ...
            profiles = cyto_profiles[cyto_idx]
            ave_profiles = profiles.mean(axis=0)
            ave_means = (bin_positions * ave_profiles).sum(axis=-1) / ave_profiles.sum(
                -1
            )
        print(ave_means)
        for i, celltype_name in enumerate(celltypes):
            features[
                f"ave. {cyto_name} profile, {celltype_name} mean position"
            ] = ave_means[i]
    ...
    for c, cyto_name in enumerate(cytoarchitectures):
        ...
        if len(cyto_idx) == 0:
            ...
            ave_moment = (bin_positions ** 2 * ave_profiles).sum(
                axis=-1
            ) / ave_profiles.sum(-1)
        ...
        for i, celltype_name in enumerate(celltypes):
            features[
                f"ave. {cyto_name} profile, {celltype_name} profile stdev"
            ] = ave_stdev[i]
...
def wholeorg_features(args, features, gate_labels, niche_labels):
    ...
    if not np.allclose(voxel_down, max(voxel_down)):
        ...
        foreground = resize(foreground, output_shape=shape_isotropic, order=0).astype(
            foreground.dtype
        )
    ...
    if not np.allclose(voxel_down, max(voxel_down)):
        ...
        ventricles = resize(
            ventricles, output_shape=shape_isotropic, order=0, preserve_range=True
        ).astype(ventricles.dtype)
    ...
    verbose_print(
        args, f"Found {nb_ventricles} connected components in ventricle segmentation"
    )
    ...
    print(
        f"ave. ventricle equivalent diameter (um): {ave_eq_diam_um:.3f} ({stdev_eq_diam_um:.3f})"
    )
    print(
        f"ave. ventricle major axis length (um): {ave_major_axis_um:.3f} ({stdev_major_axis_um:.3f})"
    )
    print(
        f"ave. ventricle minor axis length (mm): {ave_minor_axis_um:.3f} ({stdev_minor_axis_um:.3f})"
    )
    ...
    for n, niche_name in enumerate(niches):
        ...
        for c, celltype_name in enumerate(celltypes):
            ...
            print(
                f"Ave. surface dist: {ave_surface_dist:.3f} ({stdev_surface_dist:.3f}, n = {len(celltype_idx)})"
            )
            features[
                f"{niche_name} nbrhd, {celltype_name} surface distance mean (um)"
            ] = ave_surface_dist
            features[
                f"{niche_name} nbrhd, {celltype_name} surface distance stdev (um)"
            ] = stdev_surface_dist
...
def features_main(args):
    ...
    if os.path.isdir(args.input):
        ...
        parent_dir = os.path.abspath(
            os.path.join(os.path.abspath(args.input), os.pardir)
        )
        input_folders = [
            os.path.join(parent_dir, t, f)
            for t, f in zip(analysis["type"], analysis.index)
        ]
    else:
        raise ValueError(
            "Input must be a folder with a symlinked dataset or an analysis CSV file"
        )
    ...
    for input_folder in input_folders:
        verbose_print(
            args,
            f"Calculating multiscale features for {os.path.basename(input_folder)}",
        )
        ...
        nuclei_morphologies = pd.read_csv(
            os.path.join(args.input, "dataset/nuclei_morphologies.csv")
        )
        niche_proximities = np.load(
            os.path.join(args.input, "dataset/niche_proximities.npy")
        )
...
def features_cli(subparsers):
    features_parser = subparsers.add_parser(
        "features",
        help="Compute multiscale features",
        description="Compute multiscale features for an organoid",
    )
    features_parser.add_argument(
        "input", help="Path to input organoid folder or analysis CSV file"
    )
    features_parser.add_argument(
        "-d",
...
def combine_main(args):
    ...
    if os.path.splitext(args.inputs[0])[1] == ".csv":
        ...
        parent_dir = os.path.abspath(
            os.path.join(os.path.abspath(args.inputs[0]), os.pardir)
        )
        args.inputs = [
            os.path.join(parent_dir, t, f)
            for t, f in zip(analysis["type"], analysis.index)
...
def combine_cli(subparsers):
    combine_parser = subparsers.add_parser(
        "combine",
        help="Combine organoid features",
        description="Combine organoid features for a study",
    )
    combine_parser.add_argument(
        "inputs", help="Path to input organoid folders or analysis CSV file", nargs="+"
    )
    combine_parser.add_argument(
        "--output",
        "-o",
        help="Path to output Excel table",
        default="combined_features.xlsx",
    )
    combine_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def setup_main(args):
    ...
    for path in df.index:
        ...
        os.symlink(
            os.path.join(os.path.abspath(args.datasets), path),
...
def setup_cli(subparsers):
    ...
    setup_parser.add_argument(
        "--output", help="Path to target analysis directory", default="."
    )
    setup_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def select_cli(subparsers):
    select_parser = subparsers.add_parser(
        "select",
        help="Select datasets for analysis",
    ...
    select_parser.add_argument(
        "groups", help="Names of dataset types to select", nargs="+"
    )
    select_parser.add_argument(
        "--output", help="Path to output CSV", default="analysis.csv"
    )
    select_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def multiscale_cli(subparsers):
    multiscale_parser = subparsers.add_parser(
        "multiscale",
        help="multiscale features",
        description="Build features for multiscale organoid analysis",
    )
    multiscale_parser = multiscale_parser.add_subparsers(
        dest="multiscale_command", title="multiscale subcommands"
    )
