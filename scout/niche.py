...
def radial_profile(pts, distances, indices, radius, bins, labels=None):
    ...
    vbin_edges, vbin_width = make_bins(
        0, 4 / 3 * (radius + 1e-6) ** 3, bins
    )  # Equal-volume concentric shells
    bin_edges = (3 / 4 * vbin_edges) ** (1 / 3)

    for n, (pt, idx, dist) in tqdm(
        enumerate(zip(pts, indices, distances)), total=len(pts)
    ):
...
def proximity(centroids, celltypes, k, radius):
    ...
    ave_distances = np.asarray(
        [distances.mean(axis=-1) for distances in distances_list]
    ).T
    ...
    proximities = np.asarray(
        [
            (1 / (1 + dist / r)).prod(axis=-1)
            for (dist, r) in zip(distances_list, radius)
        ]
    ).T
...
def radial_main(args):
    ...
    for i, labels in enumerate(celltypes.T):
        ...
        profiles[i] = radial_profile(
            centroids, distances, indices, args.r, args.b, labels
        )
...
def radial_cli(subparsers):
    radial_parser = subparsers.add_parser(
        "radial",
        help="Calculate radial profiles",
        description="Calculates radial profiles for each cell-type",
    )
    radial_parser.add_argument(
        "centroids", help="Path to input nuclei centroids in micron numpy array"
    )
    radial_parser.add_argument(
        "celltypes", help="Path to input cell-type labels numpy array"
    )
    ...
    radial_parser.add_argument(
        "-b", help="Number of bins in profile", type=float, default=5
    )
    radial_parser.add_argument(
...
def proximity_cli(subparsers):
    proximity_parser = subparsers.add_parser(
        "proximity",
        help="Calculate cell-type proxiomities",
    ...
    proximity_parser.add_argument(
        "-r",
        help="Reference radii in micron for each cell-type",
        type=float,
        nargs="+",
        default=None,
    )
    proximity_parser.add_argument(
        "-k", help="Number of neighbors in proximity", type=int, default=3
    )
    proximity_parser.add_argument(
        "-p", "--plot", help="Flag to show plots", action="store_true"
    )
    proximity_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def gate_main(args):
    ...
    mid_left = np.logical_and(
        np.logical_and(low_top, np.logical_not(high_top)), np.logical_not(low_right)
    )
    mid_right = np.logical_and(
        np.logical_and(low_right, np.logical_not(high_right)), np.logical_not(low_top)
    )
    mid_inter = np.logical_and(
        np.logical_and(low_right, np.logical_not(high_right)),
        np.logical_and(low_top, np.logical_not(high_top)),
    )

    onehot = np.asarray(
        [btm_left, btm_right, top_left, top_right, mid_left, mid_right, mid_inter]
    )
    labels = np.argmax(onehot, axis=0)
    ...
    if args.plot:
        ...
        for i in range(len(names)):
            ...
            if len(idx) == 0:
                continue
            plt.plot(
                proximities_sample[idx, 0],
                proximities_sample[idx, 1],
                ".",
                alpha=args.alpha,
                label=names[i],
            )
...
def gate_cli(subparsers):
    gate_parser = subparsers.add_parser(
        "gate",
        help="Gate cells into distinct niches",
    ...
    gate_parser.add_argument(
        "--low", help="Low proximity threshold", nargs="+", type=float, required=True
    )
    gate_parser.add_argument(
        "--high", help="High proximity threshold", nargs="+", type=float, required=True
    )
    gate_parser.add_argument(
        "-p", "--plot", help="Flag to show plots", action="store_true"
    )
    gate_parser.add_argument(
        "-a", "--alpha", help="Flag to show plots", type=float, default=0.01
    )
    gate_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def combine_cli(subparsers):
    combine_parser = subparsers.add_parser(
        "combine",
        help="Combine data from multiple organoids",
...
def tsne_main(args):
    ...
    x_tsne = TSNE(
        n_components=2, n_jobs=-1, perplexity=800, learning_rate=100
    ).fit_transform(proximities)
...
def tsne_cli(subparsers):
    tsne_parser = subparsers.add_parser(
        "cluster",
        help="Cluster cells into niches",
    ...
    tsne_parser.add_argument(
        "tsne", help="Path to output t-SNE coordinates numpy array"
    )
    tsne_parser.add_argument(
        "-p", "--plot", help="Flag to show plots", action="store_true"
    )
...
def name_cli(subparsers):
    name_parser = subparsers.add_parser(
        "name",
        help="Assign names to each group",
    ...
    name_parser.add_argument(
        "--output", "-o", help="Path to output names file", required=True
    )
    name_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def niche_cli(subparsers):
    niche_parser = subparsers.add_parser(
        "niche", help="niche analysis", description="Organoid niche analysis tool"
    )
    niche_subparsers = niche_parser.add_subparsers(
        dest="niche_command", title="niche subcommands"
    )
