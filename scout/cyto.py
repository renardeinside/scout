...
def marching_cubes(seg, level, spacing, step_size):
    return marching_cubes_lewiner(
        seg, level=level, spacing=spacing, step_size=step_size, allow_degenerate=False
    )
...
def plot_nuclei(
    centers_um, nb_nuclei, sox2_labels, tbr1_labels, scale_factor=1, figure=None
):
    if figure is not None:
        mlab.figure(figure)
    centers_sample, sox2_labels_sample, tbr1_labels_sample = randomly_sample(
        nb_nuclei, centers_um, sox2_labels, tbr1_labels
    )

    negative_idx = np.where(
        np.logical_and(sox2_labels_sample == 0, tbr1_labels_sample == 0)
    )[0]
    sox2_idx = np.where(
        np.logical_and(sox2_labels_sample > 0, tbr1_labels_sample == 0)
    )[0]
    ...
    mlab.points3d(
        sox2[:, 0], sox2[:, 1], sox2[:, 2], scale_factor=scale_factor, color=(1, 0, 0)
    )
    mlab.points3d(
        tbr1[:, 0], tbr1[:, 1], tbr1[:, 2], scale_factor=scale_factor, color=(0, 1, 0)
    )
...
def compute_profile(vert, vi, ci, length, bins, radius):
    ...
    v_cross = np.array([[0, -vi[2], vi[1]], [vi[2], 0, -vi[0]], [-vi[1], vi[0], 0]])
    ...
    for j, bin_start in enumerate(bin_edges[:-1]):
        ...
        x, y, z = (
            pts_translated_rotated[:, 2],
            pts_translated_rotated[:, 1],
            pts_translated_rotated[:, 0],
        )

        idx = np.where(
            np.logical_and(
                x ** 2 + y ** 2 <= radius ** 2,
                np.logical_and(z >= bin_start, z <= bin_stop),
            )
        )[0]
...
def compute_profiles(
    verts, normals, length, bins, radius, centers_um, sox2_labels, tbr1_labels
):
    global centers_um_global
    global sox2_labels_global
    global tbr1_labels_global
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
        results = list(
            tqdm(pool.imap(_compute_profile, args_list), total=len(args_list))
        )
...
def profiles_to_features(profiles, normalize=True):
    features = profiles.reshape((len(profiles), -1)).astype(
        np.float
    )  # Flattened profiles
    if normalize:
        features = scale(
            features
        )  # Normalize each feature (cell bin) to unit mean, zero variance
    return features
...
def mesh_cli(subparsers):
    mesh_parser = subparsers.add_parser(
        "mesh",
        help="Mesh segmentation to surface",
    ...
    mesh_parser.add_argument(
        "-d",
        help="Downsampling factors from voxel size file",
        type=int,
        nargs="+",
        default=None,
    )
    ...
    mesh_parser.add_argument(
        "-l", help="Isolevel for surface after smoothing", type=float, default=0.2
    )
...
def profiles_main(args):
    ...
    profiles = compute_profiles(
        verts, normals, args.l, args.b, args.r, centroids_um, sox2_labels, tbr1_labels
    )
...
def profiles_cli(subparsers):
    profiles_parser = subparsers.add_parser(
        "profiles",
        help="Compute profiles along normals",
    ...
    profiles_parser.add_argument(
        "-l", help="Length in micron of the profiles", type=float, default=300
    )
    profiles_parser.add_argument(
        "-r", help="Radius of profile bins", type=int, default=25
    )
    profiles_parser.add_argument(
        "-b", help="Number of bins in each profile", type=int, default=6
    )
    profiles_parser.add_argument(
        "-p", "--plot", help="Flag to show plot", action="store_true"
    )
...
def sample_cli(subparsers):
    ...
    sample_parser.add_argument(
        "-i", "--inputs", help="Path to input numpy arrays", nargs="+", required=True
    )
    sample_parser.add_argument(
        "-o",
        "--outputs",
        help="Path to sampled output numpy arrays",
        nargs="+",
        required=True,
    )
...
def combine_main(args):
    ...
    paths = [
        os.path.join(parent_dir, df.loc[folder]["type"], folder, "dataset", args.name)
    ...
    verbose_print(
        args, f"Saving combined features to {args.output} with shape {combined.shape}"
    )
...
def combine_cli(subparsers):
    combine_parser = subparsers.add_parser(
        "combine",
        help="Combine sampled cytoarchitectures",
    ...
    combine_parser.add_argument(
        "-o", "--output", help="Path to output combined numpy array", required=True
    )
    combine_parser.add_argument(
        "-s", "--sample", help="Path to output with sample name", required=True
    )
    combine_parser.add_argument(
        "-n",
        "--name",
        help="Filename of sampled profiles",
        default="cyto_profiles_sample.npy",
    )
...
def cluster_cli(subparsers):
    cluster_parser = subparsers.add_parser(
        "cluster",
        help="Cluster cytoarchitectures",
    ...
    cluster_parser.add_argument(
        "-p", "--plot", help="Plotting flag", action="store_true"
    )
    cluster_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def classify_main(args):
    ...
    x_train = profiles_to_features(
        profiles_train, normalize=False
    )  # Normalizes the data (should we do this?)
    ...
    for c in classes:
        ...
        verbose_print(
            args, f"Class {c}: {count:10d} profiles {100 * count / nb_cells:10.3f}%"
        )
...
def classify_cli(subparsers):
    classify_parser = subparsers.add_parser(
        "classify",
        help="Cluster profiles into cyto labels",
        description="Clusters profiles into cytoarchitectures based on profiles",
    )
    classify_parser.add_argument(
        "profiles_train", help="Path to input profiles numpy array for training"
    )
    classify_parser.add_argument(
        "labels_train", help="Path to output cyto labels numpy array for training"
    )
    classify_parser.add_argument(
        "profiles", help="Path to input profiles numpy array to classify"
    )
    classify_parser.add_argument(
        "labels", help="Path to output cyto labels numpy array"
    )
    classify_parser.add_argument(
        "--umap", help="Path to fit UMAP embedding", default=None
    )
    classify_parser.add_argument(
        "--save", help="Path to save trained model", default=None
    )
    classify_parser.add_argument(
        "--load", help="Path to load a trained model", default=None
    )
    classify_parser.add_argument(
        "-v", "--verbose", help="Verbose flag", action="store_true"
    )
...
def cyto_cli(subparsers):
    cyto_parser = subparsers.add_parser(
        "cyto",
        help="cytoarchitecture analysis",
        description="Organoid cytoarchitecture analysis",
    )
    cyto_subparsers = cyto_parser.add_subparsers(
        dest="cyto_command", title="cyto subcommands"
    )
